...
XS 是一个用来在 Perl 和需要在 Perl 内使用的 C 代码（或者 C 库）之间创
建扩展的接口描述文件格式。XS 接口与 C 库连接创建了一个能被 Perl 动态导
入或者静态链接到 Perl 的新的库。XS 接口描述是用 XS 语言写的，是 Perl
扩展接口的核心组成。
 
一个 B<XSUB> 是构成 XS 接口的最基本单位。通过 B<xsubpp> 编译器的编译，
每个 XSUB 等价于一个 C 函数的定义，这粘合了 Perl 调用的规定和 C 调用的
规定。

这个粘合代码将参数从 Perl 的参数栈中取出，将这些 Perl 值转化成 C 函数的
格式，调用 C 函数，将返回值返回到 Perl 中。这里的返回值可以是那些能够作
为输出参数的 C 类型的返回值或者任何 C 函数的参数。这些返回值可以通过放
到 Perl 参数栈或者修改 Perl 提供的参数来返回给 Perl。

上面所说的大概就是将要发生的事情的简单描述。由于 Perl 的调用规则比 C
要灵活得多，因此 XSUB 实际上要做更多的事情，例如检验输入参数的合法性，
如果 C 函数的返回值表示失败的话要抛出一个异常（或者返回 undef/空列
表），根据参数的数量和类型调用不同的 C 函数，提供面向对象的接口等等。

当然，你可以直接使用 C 来写这样的粘合代码。但是这将是一个冗长乏味的工
作。特别是当你需要为多个 C 函数写这样的粘合代码，而其中一个与 Perl 的
参数栈规则或者其它难懂的东西（arcana）不是很相似的时候。 XS 可以在这一
点上能帮助你：不用写很长的粘合 C 代码，你可以只写一个更简短的对粘合代
码完成任务的I<说明>，让 XS 的编译器 B<xsubpp> 来处理其余的事情。

XS 语言允许你描述 C 函数的使用和对应 Perl 函数使用之间的映射关系。也允
许创建一个直接转换成 C 代码的 Perl 函数，这个函数可以是一个不存在的 C
函数。当 C 接口与 Perl 接口一致时，XSUB 的声明几乎和 C 函数的声明（K&R
风格）完全相同。在这种情况下，有一个叫 C<h2xs> 的工具能把一个 C 头文件
转换成对应的 XS 文件。在这个 XS 文件中提供了头文件中的函数和宏的粘合代
码。

XS 的编译器叫做 B<xsubpp>。这个编译器创建能让 XSUB 操作 Perl 数据的结
构（construct），以及能让 Perl 调用 XSUB 的粘合代码。它使用 B<typemap>
来映射 C 函数的参数和返回给 Perl 数据。默认的 typemap （和 Perl 一起发
行）能处理许多普通的 C 类型。对于特殊的结构体和要链接的库中的数据类型
可能要提供对应的 typemap。

XS 格式的文件是以 C 语言部分开始，直到第一个 C<MODULE = Z<>> 指令为止。
其它 XS 指令和 XSUB 定义是从这一行开始的。文件的这一部分作用的语言通常
称谓 XS 语言。B<xsubpp> 能够识别并跳过在 C 和 XS 语言部分中的 POD（参
考 L<perlpod>），这样就允许在 XS 文件中嵌入文档。

如果想要一个创建扩展的指南，可以参考 L<perlxstut>。

注意：对于一些扩展，Dave Beazley 的 SWIG 系统可能提供一个非常简便的机
制来创建粘合代码。请参考 http://www.swig.org/。

=head2 让我们开始吧

下面的很多例子将集中于创建 Perl 与 ONC+ RPC bind 库函数的接口。
rpcb_gettime() 函数用于演示 XS 语言的许多特性。这个函数有两个参数：一
个是输入参数，另一个是输出参数。这个函数不返回一个状态值。
...
如果 XSUB 提供从这个函数到 Perl 的直接转换，就可以在 Perl 中通过下面的
代码来使用这个 XSUB，$status 和 $timep 变量将保存函数的输出：

     use RPC;
     $status = rpcb_gettime( "localhost", $timep );

下面的 XS 文件就是一个 XS 函数（XS subroutine），或者称之为 XSUB。它演
示了一个可能的rpcb_gettime() 函数的接口。这个 XSUB 代表一个由 C 到
Perl 的直接转换。在 Perl 中仍然保留了原来的接口。这个 XSUB 将可以在
Perl 中用上面显示的方法来调用。注意前三个 #include 语句，引入
C<EXTERN.h>，C<perl.h>，和 C<XSUB.h>。这总是会出现在 XS 文件的开头的。
这在后面会进一步说明。
...
包括 XSUB 在内 Perl 的任何扩展，都应该有一个 Perl 模块作为引导程序将扩
展导入 Perl。这个模块导出扩展的函数和变量到 Perl 程序，使得扩展的 XSUB
能够链接到 Perl。在本文中的大部分例子将使用下面的模块。正如前面显示的
那样，在 Perl 中可以用 C<use> 命令导入模块。后面会详细解释 Perl 模块的
概念。
...
本文开发出多种针对 rpcb_gettime() 的 XSUB 接口。这些 XSUB 将接受不同顺
序的参数或者接受不同数目的参数。对于每个例子，XSUB 是 C 的
rpcb_gettime() 函数和 Perl 之间的抽象层。XSUB 必须确保真正的
rpcb_gettime() 函数是以正确的方式被调用。这个抽象层可以使编程人员创造
出更像 Perl 的接口，而不是原来的 C 函数。

=head2 The Anatomy of an XSUB

最简单的 XSUB 由 3 个部分组成：返回值的类型，XSUB 的函数名和参数名，以
及参数的类型描述也称参数的格式。

下面的 XSUB 允许 Perl 程序访问 C 库函数 sin()。这个 XSUB 模仿 C 函数只
有一个参数并返回一个值。
...
你也可以把类型描述和参数名列表合并在一起，像这样重写：

     double
     sin(double x)

这使让 XSUB 看上去更类似于 ANSI C 的声明。在参数列表之后不可以有一个分
号，像这样：

     double
     sin(double x);

C 指针参数有两种语义，C 函数是用类似的声明：

     bool string_looks_as_a_number(char *s);
     bool make_char_uppercase(char *c);

这在 XS 中是不行的。这些函数的参数需要向 B<xsubpp> 这样描述：

     char *  s
     char    &c

这些 XS 的声明都对应于 C<char*> 的 C 类型，但是它们有不同的语义，参见
L<“& 单目运算符”>。

你可以把间接引用运算符 C<*> 看作类型的一部分，而取址运算符 C<&> 当作变
量的一部分，这样可能更容易明白一些。参见 L<“Typemap”> 可以了解更多关于
处理 C 类型中限定词和单目运算符的内容。

函数名和返回值类型必须放在不同的行中，而且必须是左对齐的。
...
函数其它描述可以缩进或者左对齐。下一个例子显示一个左对齐函数的主体部分。
本文中大多数例子为了使代码更容易读而采用缩进的方式。
...
更复杂的 XSUB 可能含有更多的部分。XSUB 的每个部分都是以对应的关键字开
始的，例如 INIT: 或者 CLEANUP:。但是 XSUB 的头两行总是一样的：返回值的
描述和函数名及参数。除非显示的使用其它关键字标记，接下来应该是 INPUT:
部分。（参考 L<INPUT: 关键字>）。

一个 XSUB 部分直到另一个部分开始的关键字出现才结束。

=head2 参数栈

Perl 的参数栈用来存储传递给 XSUB 的参数值以及 XSUB 的返回值。事实上，
所有的 Perl 函数（包括那些不是 XSUB 的函数）都是同时使用这个栈来保存参
数值，每个函数的参数值都被限制放置于它们在栈中的区间内。栈的第一个位置
属于激活的函数，在本文中，我们称之为函数的 position 0。

XSUB 通过使用 B<ST(x)> 宏得到栈中的参数，这里 I<x> 是指 XSUB 在栈中的
位置。函数的 position 0 对于 XSUB 来说就是 ST(0)。对于大多数简单的情
况，B<xsubpp> 编译器能够从 typemap 中由内嵌的代码片断产生代码来处理参
数栈。对于复杂的情况，程序员必须提供这些代码。

=head2 RETVAL 变量

RETVAL 是一个自动为你定义的特殊 C 变量。
