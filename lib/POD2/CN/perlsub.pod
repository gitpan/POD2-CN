...
子程序的声明和定义：
    ...
    sub NAME;                       #  简单的声明
    sub NAME(PROTO);                #  同上，但是包含原型描述
    sub NAME : ATTRS;               #  带属性描述的声明
    sub NAME(PROTO) : ATTRS;        #  同时包含属性描述和原型描述

    sub NAME BLOCK                  #  声明的同时进行定义
    sub NAME(PROTO) BLOCK           #  同上，但是包含原型描述
    sub NAME : ATTRS BLOCK          #  带属性描述的定义
    sub NAME(PROTO) : ATTRS BLOCK   #  同时包含属性描述和原型描述

在运行时定义一个匿名的子程序：

    $subref = sub BLOCK;            # 不包含属性描述和原型描述
    $subref = sub (PROTO) BLOCK;    # 包含原型描述
    $subref = sub : ATTRS BLOCK;    # 包含属性描述
    $subref = sub (PROTO) : ATTRS BLOCK; # 同时包含属性描述和原型描述
